                ...
                for subw in range(kernel_size):
                    for subin in range(n_in):
                        for subout in range(out_shape):
                            ...
                            for b in range(bs):
                                output[b, i, j, subout] += (
                                    x_pad[b, i + subh, j + subw, subin] * w[subh, subw, subin, subout]
                                )
    return output
...
class ConvNp:
    ...
    def forward(self, x):
        ...
        x_pad = np.concatenate(
            (np.zeros((bs, h + self.pad_size * 2, 1, n_in)), x_pad, np.zeros((bs, h + self.pad_size * 2, 1, n_in))),
            axis=2,
        )
        self.x_last = x_pad
        #         if self.nesterov:
        ...
        output = optimized_conv_f(x_pad, self.w, bs, h, w, self.kernel_size, n_in, self.out_shape)
    ...
    def backward(self, err, lr):
        if 1:
            ...
            grad_backward = optimized_conv_f(err_padded, rotated_w, bs, h, w, self.kernel_size, n_out, self.in_shape)
            ...
            e_pad = np.concatenate(
                (
                    np.zeros((bs, h + self.pad_size * 2, 1, n_out)),
                    e_pad,
                    np.zeros((bs, h + self.pad_size * 2, 1, n_out)),
                ),
                axis=2,
            ...
            for i in range(h):
                for j in range(w):
                    ...
                    grad_w_raw = np.sum(
                        e_pad[:, i : i + 1, j : j + 1, None, :] * self.x_last[:, h_start:h_end, w_start:w_end, :, None],
                        axis=0,
                    )
        ...
        if self.momentum == 0:
            ...
            self.w_vel = self.w_vel * self.momentum + grad_w * (1 - self.momentum) * lr
            self.b_vel = self.b_vel * self.momentum + beta_update * (1 - self.momentum) * lr
            self.w += self.w_vel
            self.b += self.b_vel
